...
test('Cart opens when user clicks shopping cart icon', async ({ page }) => {
  ...
  await takeFinalScreenshot(page, 'cart')
})

test('When shopping cart contains at least 1 product all buttons enabled and directs to correct links', async ({
  page
}) => {
  ...
  await takeFinalScreenshot(page, 'checkout')
})

// BUG - TEST FAILED, reported in jira <jira link>. Remove fixme when fixed
test.fixme(
  'When shopping cart is empty, checkout button is disabled',
  async ({ page }) => {
    await clickElement(page, buttonsData.shoppingCart)
    ...
    await confirmURL(
      ...
      pageData.pageTitle,
      'Your Cart'
    ) // this should be visible, when button inactive
    await takeFinalScreenshot(page, 'checkoutBtnDisabled')
  }
)

test('When shopping cart is empty, continue shopping button is enabled and directs to corret tab', async ({
  page
}) => {
  await clickElement(page, buttonsData.shoppingCart)
  ...
  await takeFinalScreenshot(page, 'continueShopping')
})

test('When the shopping cart contains at least one product, the user is directed to the checkout', async ({
  page
}) => {
  await isProductAddedToCart(page)
  await clickElement(page, buttonsData.shoppingCart)
  ...
  await takeFinalScreenshot(page, 'shoppingCartcheckout')
})

test('When user is in checkout tab can finish shopping process', async ({
  page
}) => {
  await isProductAddedToCart(page)
  ...
  await confirmURL(
    page,
    cartData.checkoutUrl2,
    pageData.pageTitle,
    'Checkout: Overview'
  )

  const cartValue = await compareQuantity(
    page,
    cartData.cartUrl,
    'div.cart_quantity'
  )
  const checkoutValue = await compareQuantity(
    page,
    cartData.checkoutUrl2,
    'div.cart_quantity'
  )
  ...
  await takeFinalScreenshot(page, 'checkoutComplete')
})
...
test('User directed back to cart when checkout cancelled', async ({ page }) => {
  ...
  await confirmURL(page, cartData.cartUrl, pageData.pageTitle, 'Your Cart')
  await takeFinalScreenshot(page, 'backToCart')
})
